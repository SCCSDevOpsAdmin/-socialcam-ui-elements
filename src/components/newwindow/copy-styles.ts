...
export const getStylesToLoad = (sourceDoc: Document, targetDoc: Document) => {
    let sourceDocStylesheets: CSSStyleSheet[] = [];
    let toLoad: CSSStyleSheet[] = [];

    Array.from(sourceDoc.styleSheets).forEach((styleSheet) =>
        sourceDocStylesheets.push(styleSheet)
    );
