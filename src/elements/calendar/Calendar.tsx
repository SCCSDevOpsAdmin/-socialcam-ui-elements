...
export const Calendar = React.forwardRef((props: CalendarProps, ref) => {
    ...
    const onInputFocus = (event) => {
        if (ignoreFocusFunctionality.current) {
            ...
            if (props.showOnFocus && !props.showIcon && !visible) {
                show();
            }
    ...
    const validEightDigitDate = (str: any) => {
    ...
    const updateValueOnInput = (event, rawValue) => {
        if (rawValue.length == 8) {
            const value_ = validEightDigitDate(rawValue);
            if (!isNaN(value_.valueOf()) && isValidSelection(value_)) {
                updateModel(event, value_);
                updateViewDate(event, value_);
                return;
            }
        }
        try {
    ...
    const updateTime = (event, hour, minute, second, millisecond) => {
        ...
        if (isMultipleSelection() && Array.isArray(props.value)) {
        ...
        } else if (isRangeSelection() && Array.isArray(props.value)) {
            if (props.value && props.value.length) {
                let startDate = props.value[0];
                let endDate = props.value[1];
    ...
    const isNavIconClicked = (target) => {
        return (
            ...
            DomHandler.hasClass(target, 'cs-datepicker-month') ||
            DomHandler.hasClass(target, 'cs-monthpicker-month') ||
            DomHandler.hasClass(target, 'cs-datepicker-year') ||
            DomHandler.hasClass(target, 'cs-yearpicker-year') ||
            DomHandler.hasClass(target, 'cs-monthpicker') ||
            DomHandler.hasClass(target, 'cs-yearpicker')
        );
    };
    ...
    const isSelected = (dateMeta) => {
        if (props.value) {
            ...
            } else if (isMultipleSelection() && Array.isArray(props.value)) {
                let selected = false;
    ...
    const setNavigationState = (newViewDate) => {
        ...
        if (props.disabled) {
            DomHandler.addClass(navPrev, 'cs-disabled');
            DomHandler.addClass(navNext, 'cs-disabled');
        ...
        if (props.minDate) {
            ...
            if (props.minDate > firstDayOfMonth) {
                ...
                DomHandler.removeClass(navPrev, 'cs-disabled');
            }
        }
        ...
        if (props.maxDate) {
            ...
            if (props.maxDate < lastDayOfMonth) {
                ...
                DomHandler.removeClass(navNext, 'cs-disabled');
            }
        }
    };
    ...
    const createForwardNavigator = (isVisible) => {
        //const navigatorProps = isVisible  ? { onClick: onNextButtonClick, onKeyDown: (e) => onContainerButtonKeydown(e) }
        //    : { style: { visibility: 'hidden' } };
        //return (
        //    <button type="button" className="cs-datepicker-next" {...navigatorProps}>
    ...
    const createTitle = (monthMetaData) => {
        ...
        const month = createTitleMonthElement();
    ...
    const createMonths = (monthsMetaData) => {
        ...
        return <div className="cs-datepicker-group-container">{groups}</div>;
    };
                    ...
                    {monthPickerValues().map((m, i) => {
                        return (
                            <span
                                ...
                                key={`month${i + 1}`}
                                className={classNames('cs-monthpicker-month', {
                    ...
                    {yearPickerValues().map((y, i) => {
                        return (
                            <span
                                ...
                                key={`year${i + 1}`}
                                className={classNames('cs-yearpicker-year', {
...
Calendar.defaultProps = {
    ...
    dateTemplate: undefined,
    decadeTemplate: undefined,
    headerTemplate: undefined,
    footerTemplate: undefined,
    monthNavigatorTemplate: undefined,
    yearNavigatorTemplate: undefined,
