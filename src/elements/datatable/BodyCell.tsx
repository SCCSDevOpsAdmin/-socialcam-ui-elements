...
export const BodyCell = React.memo((props: any) => {
    ...
    const getColumnProp = (prop) =>
        props.column ? props.column.props[prop] : null;
    ...
    const findNextSelectableCell = (cell) => {
        ...
        return nextCell
            ? DomHandler.hasClass(nextCell, 'cs-selectable-cell')
                ? nextCell
    ...
    const findPrevSelectableCell = (cell) => {
        ...
        return prevCell
            ? DomHandler.hasClass(prevCell, 'cs-selectable-cell')
                ? prevCell
    ...
    const findDownSelectableCell = (cell) => {
        ...
        return downRow && downCell
            ? DomHandler.hasClass(downRow, 'cs-selectable-row') &&
              DomHandler.hasClass(downCell, 'cs-selectable-cell')
                ? downCell
    ...
    const findUpSelectableCell = (cell) => {
        ...
        return upRow && upCell
            ? DomHandler.hasClass(upRow, 'cs-selectable-row') &&
              DomHandler.hasClass(upCell, 'cs-selectable-cell')
                ? upCell
    ...
    const createElement = () => {
        ...
        const className = classNames(
            ...
            cellClassName,
            {
                'cs-selection-column': selectionMode !== null,
                'cs-editable-column': editor,
                'cs-cell-editing': editor && editingState,
        ...
        if (selectionMode) {
            ...
            content = showReorder && (
                <i
                    className={classNames(
                        'cs-datatable-reorderablerow-handle',
            ...
            const rowEditorPropsEditingState = {
                editing: true,
                onSaveClick: onRowEditSave,
                saveClassName: 'cs-row-editor-save cs-link',
                saveIconClassName:
                    'cs-row-editor-save-icon csi csi-fw csi-check',
                onCancelClick: onRowEditCancel,
                cancelClassName: 'cs-row-editor-cancel cs-link',
                cancelIconClassName:
                    'cs-row-editor-cancel-icon csi csi-fw csi-times',
            };

            const rowEditorPropsNonEditingState = {
                editing: false,
                onInitClick: onRowEditInit,
                initClassName: 'cs-row-editor-init cs-link',
                initIconClassName:
                    'cs-row-editor-init-icon csi csi-fw csi-pencil',
            };
            ...
            if (editingState) {
                content = (
                    <>
                        <button
                            type="button"
                            name="row-save"
                            onClick={rowEditorPropsEditingState.onSaveClick}
                            className={rowEditorPropsEditingState.saveClassName}
                            tabIndex={props.tabIndex}
                        >
                            <span
                                className={
                                    rowEditorPropsEditingState.saveIconClassName
                                }
                            ></span>
                            <Ripple />
                        </button>
                        <button
                            type="button"
                            name="row-cancel"
                            onClick={rowEditorPropsEditingState.onCancelClick}
                            className={
                                rowEditorPropsEditingState.cancelClassName
                            }
                            tabIndex={props.tabIndex}
                        >
                            <span
                                className={
                                    rowEditorPropsEditingState.cancelIconClassName
                                }
                            ></span>
                            <Ripple />
                        </button>
                ...
                content = (
                    <button
                        type="button"
                        name="row-edit"
                        onClick={rowEditorPropsNonEditingState.onInitClick}
                        className={rowEditorPropsNonEditingState.initClassName}
                        tabIndex={props.tabIndex}
                    >
                        <span
                            className={
                                rowEditorPropsNonEditingState.initIconClassName
                            }
                        ></span>
                        <Ripple />
                    </button>
            ...
            if (body) {
                if (editingState) {
                    rowEditorPropsEditingState['element'] = content;
                    content = ObjectUtils.getJSXElement(body, props.rowData, {
                        column: props.column,
                        field: field,
                        rowIndex: props.rowIndex,
                        frozenRow: props.frozenRow,
                        props: props.tableProps,
                        rowEditor: rowEditorPropsEditingState,
                    });
                } else {
                    rowEditorPropsNonEditingState['element'] = content;
                    content = ObjectUtils.getJSXElement(body, props.rowData, {
                        column: props.column,
                        field: field,
                        rowIndex: props.rowIndex,
                        frozenRow: props.frozenRow,
                        props: props.tableProps,
                        rowEditor: rowEditorPropsNonEditingState,
                    });
                }
            }
        ...
        if (!isRowEditor && editor) {
            ...
            editorKeyHelper = (
                <a
                    tabIndex={parseInt('0')}
                    ref={keyHelperRef}
                    className="cs-cell-editor-key-helper cs-hidden-accessible"
