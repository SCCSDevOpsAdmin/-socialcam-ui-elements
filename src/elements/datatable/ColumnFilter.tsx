...
export const ColumnFilter = React.memo((props: any) => {
    ...
    const filterModel = props.filters ? props.filters[field] : null;
    ...
    const hasFilter = () => {
        ...
        return filterStoreModel.operator
            ? !isFilterBlank(filterModel.constraints[0].value) &&
                  filterStoreModel.constraints[0].value !==
                      filterModel.constraints[0].value
            : !isFilterBlank(filterModel.value) &&
                  filterStoreModel.value !== filterModel.value;
    };
    ...
    const findDataType = () => {
        ...
        if (matchMode === 'custom' && !hasMatchMode(dataType)) {
            ...
            return (
                Object.keys(PrimeReact.filterMatchModeOptions).find((key) =>
                    hasMatchMode(key)
                ) || dataType
            );
        }
    ...
    const findNextItem = (item) => {
        ...
        return nextItem
            ? DomHandler.hasClass(nextItem, 'cs-column-filter-separator')
    ...
    const findPrevItem = (item) => {
        ...
        return prevItem
            ? DomHandler.hasClass(prevItem, 'cs-column-filter-separator')
    ...
    const createRowFilterElement = () => {
        if (props.display === 'row') {
            ...
            return (
                <div className="cs-fluid cs-column-filter-element">
                    {content}
                </div>
            );
        }
    ...
    const createMenuButton = () => {
        if (showMenuButton()) {
            const className = classNames(
                'cs-column-filter-menu-button cs-link',
                {
                    'cs-column-filter-menu-button-open': overlayVisibleState,
                    'cs-column-filter-menu-button-active': hasFilter(),
                }
            );
            ...
            return (
                ...
                >
                    <span className="csi csi-filter-icon csi-filter"></span>
                </button>
            );
        }
    ...
    const createClearButton = () => {
        if (getColumnProp('showClearButton') && props.display === 'row') {
            const className = classNames(
                'cs-column-filter-clear-button cs-link',
                {
                    'cs-hidden-space': !hasRowFilter(),
                }
            );
            ...
            return (
                <button
                    ...
                    onClick={clearFilter}
                >
                    <span className="csi csi-filter-slash"></span>
                </button>
            );
        }
    ...
    const createRowItems = () => {
        if (isShowMatchModes()) {
            ...
            return (
                <ul className="cs-column-filter-row-items">
                    {_matchModes.map((matchMode, i) => {
                        const { value, label } = matchMode;
                        const className = classNames(
                            'cs-column-filter-row-item',
                            { 'cs-highlight': isRowMatchModeSelected(value) }
                        );
                    ...
                    <li className="cs-column-filter-separator"></li>
                    <li
                        className="cs-column-filter-row-item"
                        onClick={clearFilter}
    ...
    const createOperator = () => {
        if (isShowOperator()) {
            ...
            return (
                <div className="cs-column-filter-operator">
                    <Dropdown
                        options={options}
                        value={value}
                        onChange={onOperatorChange}
                        className="cs-column-filter-operator-dropdown"
                    />
                </div>
            );
        }
        ...
        if (isShowMatchModes()) {
            ...
            return (
                <Dropdown
                    ...
                    className="cs-column-filter-matchmode-dropdown"
                />
            );
        }
        ...
        if (showRemoveIcon()) {
            ...
            return (
                <Button
                    type="button"
                    icon="csi csi-trash"
                    className="cs-column-filter-remove-button cs-button-text cs-button-danger cs-button-sm"
    ...
    const createConstraints = () => {
        ...
        return (
            <div className="cs-column-filter-constraints">
                {_fieldConstraints.map((fieldConstraint, i) => {
                    ...
                    return (
                        <div key={i} className="cs-column-filter-constraint">
                            {matchModeDropdown}
                            {menuFilterElement}
    ...
    const createAddRule = () => {
        if (isShowAddConstraint()) {
            ...
            return (
                <div className="cs-column-filter-add-rule">
                    <Button
                        type="button"
                        label={addRuleLabel}
                        icon="csi csi-plus"
                        className="cs-column-filter-add-button cs-button-text cs-button-sm"
        ...
        if (getColumnProp('showClearButton')) {
            if (!getColumnProp('filterClear')) {
                ...
                return (
                    <Button
                        type="button"
                        className="cs-button-outlined cs-button-sm"
                        onClick={clearFilter}
                        label={clearLabel}
                    />
                );
    ...
    const createButtonBar = () => {
        ...
        return (
            <div className="cs-column-filter-buttonbar">
                {clearButton}
                {applyButton}
            </div>
        );
    ...
    const createOverlay = () => {
        ...
        const className = classNames(
            'cs-column-filter-overlay cs-component cs-fluid',
            getColumnProp('filterMenuClassName'),
            {
                'cs-column-filter-overlay-menu': props.display === 'menu',
                'cs-input-filled': PrimeReact.inputStyle === 'filled',
                'cs-ripple-disabled': PrimeReact.ripple === false,
            }
        );
    ...
    const className = classNames('cs-column-filter cs-fluid', {
        'cs-column-filter-row': props.display === 'row',
        'cs-column-filter-menu': props.display === 'menu',
    });
