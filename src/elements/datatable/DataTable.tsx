...
import { DataTableProps, DataTableOperatorFilterMetaData } from './types';
import { any, number } from 'prop-types';
import './DataTable.scss';

export const DataTable = React.forwardRef(
    (props: DataTableProps | any, ref?) => {
        ...
        const [d_filtersState, setD_filtersState] = React.useState(null);
        ...
        const getMultiSortMeta = () => {
            return (
                (props.onSort ? props.multiSortMeta : multiSortMetaState) || []
            );
        };
        ...
        const getColumns = (ignoreReorderable?) => {
            ...
            if (
                ...
                let orderedColumns = columnOrderState.reduce(
                    (arr, columnKey) => {
            ...
            if (columnWidthsState.current) {
                ...
                if (ObjectUtils.isNotEmpty(widths)) {
                    ...
                    widths.forEach((width, index) => {
                        let style = props.scrollable
                            ? `flex: 1 1 ${width}px !important`
                            : `width: ${width}px !important`;
                        innerHTML += `
                        .cs-datatable[${attributeSelectorState}] .cs-datatable-thead > tr > th:nth-child(${
                            index + 1
                        }),
                        .cs-datatable[${attributeSelectorState}] .cs-datatable-tbody > tr > td:nth-child(${
                            index + 1
                        }),
                        .cs-datatable[${attributeSelectorState}] .cs-datatable-tfoot > tr > td:nth-child(${
                            index + 1
                        }) {
                            ${style}
                        }
        ...
        const onColumnResize = (event) => {
            ...
            DomHandler.addClass(elementRef.current, 'cs-unselectable-text');
        ...
        const onColumnResizeEnd = () => {
            ...
            DomHandler.removeClass(elementRef.current, 'cs-unselectable-text');
        ...
        const onColumnHeaderMouseDown = (e) => {
            ...
            if (
                ...
                if (
                    ...
                    DomHandler.hasClass(event.target, 'cs-column-resizer')
                )
        ...
        const onColumnHeaderDragStart = (e) => {
            ...
            colReorderIconHeight.current =
                DomHandler.getHiddenElementOuterHeight(
                    reorderIndicatorUpRef.current
                );

            draggedColumn.current = column;
            draggedColumnElement.current = findParentHeader(
                event.currentTarget
            );
        ...
        const onColumnHeaderDragOver = (e) => {
            ...
            if (
                ...
                if (draggedColumnElement.current !== dropHeader) {
                    ...
                    let targetLeft, columnCenter;
                    if (typeof dropHeaderOffset.left === 'string') {
                        if (typeof containerOffset.left === 'string') {
                            targetLeft =
                                parseInt(dropHeaderOffset.left) -
                                parseInt(containerOffset.left);
                        } else {
                            targetLeft =
                                parseInt(dropHeaderOffset.left) -
                                containerOffset.left;
                        }
                        if (typeof dropHeader.offsetWidth === 'string') {
                            columnCenter =
                                parseInt(dropHeaderOffset.left) +
                                parseInt(dropHeader.offsetWidth) / 2;
                        } else {
                            columnCenter =
                                parseInt(dropHeaderOffset.left) +
                                dropHeader.offsetWidth / 2;
                        }
                    } else {
                        if (typeof containerOffset.left === 'string') {
                            targetLeft =
                                dropHeaderOffset.left -
                                parseInt(containerOffset.left);
                        } else {
                            targetLeft =
                    ...
                    let tempCalc;
                    if (typeof dropHeaderOffset.top === 'string') {
                        if (typeof containerOffset.top === 'string') {
                            tempCalc =
                                parseInt(dropHeaderOffset.top) -
                                parseInt(containerOffset.top);
                        } else {
                            tempCalc =
                                parseInt(dropHeaderOffset.top) -
                                containerOffset.top;
                        ...
                        if (typeof containerOffset.top === 'string') {
                            ...
                            tempCalc =
                                dropHeaderOffset.top - containerOffset.top;
                        }
                    }

                    reorderIndicatorUpRef.current.style.top =
                        tempCalc - (colReorderIconHeight.current - 1) + 'px';
                    reorderIndicatorDownRef.current.style.top =
                        tempCalc + dropHeader.offsetHeight + 'px';
        ...
        const onColumnHeaderDrop = (e) => {
            ...
            if (draggedColumnElement.current) {
                ...
                let dropIndex = DomHandler.index(
                ...
                if (allowDrop) {
                    ...
                    let dragColIndex = columns.findIndex((child) =>
...
@media screen and (max-width: ${props.breakpoint}) {
    .cs-datatable[${attributeSelectorState}] .cs-datatable-thead > tr > th,
    .cs-datatable[${attributeSelectorState}] .cs-datatable-tfoot > tr > td {
        display: none !important;
    }

    .cs-datatable[${attributeSelectorState}] .cs-datatable-tbody > tr > td {
        display: flex;
        width: 100% !important;
        align-items: center;
    ...
    .cs-datatable[${attributeSelectorState}] .cs-datatable-tbody > tr > td:not(:last-child) {
        border: 0 none;
    }

    .cs-datatable[${attributeSelectorState}].cs-datatable-gridlines .cs-datatable-tbody > tr > td:last-child {
        border-top: 0;
        border-right: 0;
        border-left: 0;
    }

    .cs-datatable[${attributeSelectorState}] .cs-datatable-tbody > tr > td > .cs-column-title {
        display: block;
    }
}
`;
        ...
        const multisortField = (data1, data2, multiSortMeta, index) => {
            ...
            if (value1 === value2) {
                return multiSortMeta.length - 1 > index
                    ? multisortField(data1, data2, multiSortMeta, index + 1)
                    : 0;
            }
        ...
        const filterLocal = (data, filters) => {
            ...
            if (isGlobalFilter) {
                globalFilterFieldsArray =
                    ...
                    columns
                        .filter((col) => !col.props.excludeGlobalFilter)
                        .map((col) => col.props.filterField || col.props.field);
            }
            ...
            for (let i = 0; i < data.length; i++) {
                ...
                for (let prop in filters) {
                    if (
                        ...
                        if (filterMeta.operator) {
                            ...
                            isFiltered = filterMeta.value !== null;
                        }
                ...
                if (isGlobalFilter && !globalMatch && globalFilterFieldsArray) {
                    for (let j = 0; j < globalFilterFieldsArray.length; j++) {
                        ...
                        let matchMode = filters['global']
                            ? filters['global'].matchMode
                            : FilterMatchMode.CONTAINS;
                        let value = filters['global']
                            ? filters['global'].value
                            : props.globalFilter;
                        globalMatch = FilterService.filter[matchMode](
        ...
        const executeLocalFilter = (field, rowData, filterMeta, index) => {
            ...
            if (
                filterMatchMode === 'contains' &&
                dataFieldValue.includes(filterValue)
            ) {
                return true;
            }

            let filterConstraint = FilterService.filter[filterMatchMode];
        ...
        const cloneFilters = (filters: DataTableOperatorFilterMetaData) => {
        ...
        const filter = (value, field, matchMode, index = 0) => {
            ...
            let constraint =
                meta && meta.operator ? meta.constraints[index] : meta;
            ...
            if (!props.lazy) {
                if (data && data.length) {
                    ...
                    const sortColumn = columns.find(
                        (col) => col.props.field === sortField
                    );

                    if (sortColumn) {
                        columnSortable.current = sortColumn.props.sortable;
                        columnSortFunction.current =
                            sortColumn.props.sortFunction;
                    }
        ...
        const createLoader = () => {
            if (props.loading) {
                const iconClassName = classNames(
                    'cs-datatable-loading-icon csi-spin',
                    props.loadingIcon
                );

                return (
                    <div className="cs-datatable-loading-overlay cs-component-overlay">
        ...
        const createHeader = () => {
            if (props.header) {
                ...
                return <div className="cs-datatable-header">{content}</div>;
            }
        ...
        const createTableHeader = (options, empty) => {
            ...
            const filtersStore =
                (!props.onFilter && props.filters) || getFilters();
        ...
        const createTableBody = (
            ...
            const frozenBody = props.frozenValue && (
                <TableBody
                    ...
                    className="cs-datatable-frozen-tbody"
                    frozenRow
                    tableProps={props}
        ...
        const createContent = (
            ...
            return (
                <div
                    ref={wrapperRef}
                    className="cs-datatable-wrapper"
                    style={{
        ...
        const createFooter = () => {
            if (props.footer) {
                ...
                return <div className="cs-datatable-footer">{content}</div>;
            }
        ...
        const createResizeHelper = () => {
            if (props.resizableColumns) {
                return (
                    <div
                        ref={resizeHelperRef}
                        className="cs-column-resizer-helper"
                        style={{ display: 'none' }}
                    ></div>
                );
            }
                ...
                return (
                    <>
                        <span
                            ...
                            className="csi csi-arrow-down cs-datatable-reorder-indicator-up"
                            style={style}
                        ></span>
                        <span
                            ref={reorderIndicatorDownRef}
                            className="csi csi-arrow-up cs-datatable-reorder-indicator-down"
                            style={style}
                        ></span>
                    </>
                );
        ...
        const className = classNames(
            'cs-datatable cs-component',
            {
                'cs-datatable-hoverable-rows': props.rowHover,
                'cs-datatable-selectable': selectable && !props.cellSelection,
                'cs-datatable-selectable-cell':
                    selectable && props.cellSelection,
                'cs-datatable-auto-layout': props.autoLayout,
                'cs-datatable-resizable': props.resizableColumns,
                'cs-datatable-resizable-fit':
                    props.resizableColumns && props.columnResizeMode === 'fit',
                'cs-datatable-scrollable': props.scrollable,
                'cs-datatable-scrollable-vertical':
                    props.scrollable && props.scrollDirection === 'vertical',
                'cs-datatable-scrollable-horizontal':
                    props.scrollable && props.scrollDirection === 'horizontal',
                'cs-datatable-scrollable-both':
                    props.scrollable && props.scrollDirection === 'both',
                'cs-datatable-flex-scrollable':
                    props.scrollable && props.scrollHeight === 'flex',
                'cs-datatable-responsive-stack':
                    props.responsiveLayout === 'stack',
                'cs-datatable-responsive-scroll':
                    props.responsiveLayout === 'scroll',
                'cs-datatable-striped': props.stripedRows,
                'cs-datatable-gridlines': props.showGridlines,
                'cs-datatable-grouped-header': props.headerColumnGroup != null,
                'cs-datatable-grouped-footer': props.footerColumnGroup != null,
                'cs-datatable-sm': props.size === 'small',
                'cs-datatable-lg': props.size === 'large',
            },
            props.className
        );
        ...
        return (
            <div
                ...
                style={props.style}
                {...otherProps}
                data-scrollselectors=".cs-datatable-wrapper"
            >
                {loader}
                {header}
                {paginatorTop}
...
const defaultProps = {
    ...
    breakpoint: '960px',
    filterDisplay: 'menu',
    expandedRowIcon: 'csi csi-chevron-down',
    collapsedRowIcon: 'csi csi-chevron-right',
