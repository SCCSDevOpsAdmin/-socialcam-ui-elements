...
type rowGroupHeaderStyleObjectStateProps = {
    top: string;
};
...
export const TableBody = React.memo(
    React.forwardRef((props: any, ref?) => {
        ...
        ] = React.useState<rowGroupHeaderStyleObjectStateProps>(null);
        ...
        const prevVirtualScrollerOptions = usePrevious(
        ...
        const isSelectionEnabled = () => {
            return (
                ...
                (props.columns &&
                    props.columns.some(
                        (col) => col && !!col.props.selectionMode
                    ))
            );
        };
        ...
        const findIndex = (collection, rowData) => {
            return (collection || []).findIndex((data) =>
                equals(rowData.category, data.category)
            );
        };
        ...
        const updateVirtualScrollerPosition = () => {
            ...
            elementRef.current.style.top =
                (elementRef.current.style.top || 0) + tableHeaderHeight + 'px';
        };
        ...
        const onRangeSelection = (event, type) => {
            ...
            let selection = isMultipleSelection()
                ? [
                      ...new Set([
                          ...(props.selection || []),
                          ...selectionInRange,
        ...
        const enableDragSelection = (event) => {
            if (props.dragSelection && !dragSelectionHelper.current) {
                ...
                DomHandler.addClass(
                    ...
                    'cs-datatable-drag-selection-helper'
                );
            ...
            if (!allowCellSelection() && props.selectionAutoFocus) {
                if (isCheckboxSelectionModeInColumn) {
                    const checkbox = DomHandler.findSingle(
                        target,
                        'td.cs-selection-column .cs-checkbox-box'
                    );
                    checkbox && checkbox.focus();
                } else if (isRadioSelectionModeInColumn) {
                    const radio = DomHandler.findSingle(
                        target,
                        'td.cs-selection-column input[type="radio"]'
                    );
                    radio && radio.focus();
                }
            }
        ...
        const changeTabIndex = (event, type) => {
            const target = event.currentTarget;
            const isSelectable = DomHandler.hasClass(
                target,
                type === 'cell' ? 'cs-selectable-cell' : 'cs-selectable-row'
            );
        ...
        const onRowMouseDown = (e) => {
            ...
            if (allowRowDrag(e)) {
                enableDragSelection(event); //enableDragSelection(event, 'row');
        ...
        const onRowToggle = (event) => {
            ...
            let hasDataKey = props.groupRowsBy
            ...
            if (hasDataKey) {
                ...
                let expandedRowIndex = findIndex(
                    props.expandedRows,
                    event.data
                );
                expandedRows = props.expandedRows
        ...
        const onRowDragOver = (e) => {
            ...
            if (rowDragging.current && draggedRowIndex.current !== index) {
                ...
                const tempVar = DomHandler.getOffset(rowElement).top;
                let rowY;
                if (typeof tempVar === 'string') {
                    rowY = parseInt(tempVar) + DomHandler.getWindowScrollTop();
                } else {
                    rowY = tempVar + DomHandler.getWindowScrollTop();
                }
        ...
        const onRowDragLeave = (e) => {
            ...
            DomHandler.removeClass(rowElement, 'cs-datatable-dragpoint-bottom');
            DomHandler.removeClass(rowElement, 'cs-datatable-dragpoint-top');
        };
        ...
        useMountEffect(() => {
            if (
                !props.isVirtualScrollerDisabled &&
                getVirtualScrollerOption('vertical')
            ) {
        ...
        useUpdateEffect(() => {
            if (
                ...
                getVirtualScrollerOption(
                    'itemSize',
                    prevVirtualScrollerOptions
                ) !== getVirtualScrollerOption('itemSize')
            ) {
                updateVirtualScrollerPosition();
            }
        }, [props.virtualScrollerOptions]);
        ...
        const createEmptyContent = () => {
            if (!props.loading) {
                ...
                return (
                    <tr className="cs-datatable-emptymessage" role="row">
        ...
        const createGroupHeader = (rowData, index, expanded, colSpan) => {
            if (
                ...
                const content = ObjectUtils.getJSXElement(
                    ...
                    rowData,
                    { index, props: props.tableProps }
                );

                return (
                    <tr className="cs-rowgroup-header" style={style} role="row">
                        <td colSpan={colSpan}>
                            {toggler}
                            <span className="cs-rowgroup-header-name">
                                {content}
                            </span>
                        </td>
                    </tr>
        ...
        const createExpansion = (rowData, index, expanded, colSpan) => {
            if (
                ...
                return (
                    <tr
                        id={id}
                        className="cs-datatable-row-expansion"
                        role="row"
                    >
        ...
        const createGroupFooter = (rowData, index, expanded, colSpan) => {
            if (
                ...
                const content = ObjectUtils.getJSXElement(
                    ...
                    rowData,
                    { index, colSpan, props: props.tableProps }
                );

                return (
                    <tr className="cs-rowgroup-footer" role="row">
                        {content}
                    </tr>
                );
            }
        ...
        const createContent = () => {
            return props.value.map((rowData, i) => {
                const index = getVirtualScrollerOption('getItemOptions')
                    ? getVirtualScrollerOption('getItemOptions')(i).index
                    : props.first + i;
        ...
        const className = classNames('cs-datatable-tbody', props.className);
