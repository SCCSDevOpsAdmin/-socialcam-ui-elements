...
export const TableHeader = React.memo((props: any) => {
    ...
    const isColumnSorted = (column) => {
        return props.sortField !== null
            ? column.props.field === props.sortField ||
                  column.props.sortField === props.sortField
            : false;
    };
            ...
            props.columns.forEach((column) => {
                if (column.props.sortableDisabled) {
                    sortableDisabledFields.push(
                        column.props.sortField || column.props.field
                    );
    ...
    const createContent = () => {
        if (props.headerColumnGroup) {
            const rows = React.Children.toArray(
                props.headerColumnGroup.props.children
            );
            ...
            const headerRow = (
                <tr role="row">{createHeaderCells(props.columns)}</tr>
            );
            const filterRow = props.filterDisplay === 'row' && (
                <tr role="row">{createFilterCells()}</tr>
            );
    ...
    return <thead className="cs-datatable-thead">{content}</thead>;
});
