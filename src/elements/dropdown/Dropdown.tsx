...
import CsReact, { FilterService } from '../../api/Api';
import {
    useMountEffect,
    useOverlayListener,
    useUnmountEffect,
...
export const Dropdown = React.memo(
    React.forwardRef((props: DropdownProps | any, ref?) => {
        ...
        const focusInputRef = React.useRef<any>(null);
        const searchTimeout = React.useRef<any>(null);
        const searchValue = React.useRef<any>(null);
        ...
        const appendTo = props.appendTo || CsReact.appendTo;

        const [bindOverlayListener, unbindOverlayListener] = useOverlayListener(
            {
                target: elementRef,
                overlay: overlayRef,
                listener: (event: any, { type, valid }: any) => {
                    if (valid) {
                        type === 'outside'
        ...
        const isClearClicked = (event: any) => {
            return (
                DomHandler.hasClass(event.target, 'cs-dropdown-clear-icon') ||
                DomHandler.hasClass(
                    event.target,
                    'cs-dropdown-filter-clear-icon'
                )
            );
        };
        ...
        const onFilterInputKeyDown = (event: React.KeyboardEvent) => {
            switch (event.which) {
                //down
                case 40:
                    onDownKey(event);
        ...
        const onUpKey = (event: React.KeyboardEvent) => {
            if (visibleOptions) {
        ...
        const onDownKey = (event: React.KeyboardEvent) => {
            if (visibleOptions) {
        ...
        const findNextOption = (index: any): any => {
            if (props.optionGroupLabel) {
        ...
        const findNextOptionInList = (list: any, index: any): any => {
            const i = index + 1;
            if (i === list.length) {
                return null;
            }
            ...
            return isOptionDisabled(option)
                ? findNextOptionInList(list, i)
                : option;
        };

        const findPrevOption = (index: any): any => {
            if (index === -1) {
                return null;
            }
            ...
            if (props.optionGroupLabel) {
                ...
                const option: any = findPrevOptionInList(
                    getOptionGroupChildren(visibleOptions[groupIndex]),
                    optionIndex
                );
        ...
        const findPrevOptionInList = (list: any[], index: any): any => {
            const i = index - 1;
            if (i < 0) {
                return null;
            }
        ...
        const onEditableInputChange = (event: any) => {
            if (props.onChange) {
                props.onChange({
        ...
        const onFilterClearIconClick = (callback: () => any) => {
            resetFilter(callback);
        };

        const resetFilter = (callback?: () => any) => {
            setFilterState('');
        ...
        const getSelectedOptionIndex = (): any => {
            if (props.value != null && visibleOptions) {
                if (props.optionGroupLabel) {
                    for (let i = 0; i < visibleOptions.length; i++) {
                        let selectedOptionIndex = findOptionIndexInList(
                            props.value,
        ...
        const onOverlayEnter = (callback: any) => {
            ZIndexUtils.set(
                'overlay',
                overlayRef.current,
                CsReact.autoZIndex,
        ...
        const onOverlayEntered = (callback: any) => {
            callback && callback();
            bindOverlayListener();
        ...
        const updateEditableLabel = (option?: any) => {
            if (inputRef.current) {
        ...
        const getSelectedOption = () => {
            ...
            return index !== -1
                ? props.optionGroupLabel
                    ? getOptionGroupChildren(visibleOptions[index.group])[
                          index.option
                      ]
        ...
        const createHiddenSelect = () => {
            ...
            return (
                <div className="cs-hidden-accessible cs-dropdown-hidden-select">
                    <select
                        ref={inputRef}
            ...
            if (props.editable) {
                ...
                return (
                    <input
                        ...
                        type="text"
                        defaultValue={value}
                        className="cs-dropdown-label cs-inputtext"
                        disabled={props.disabled}
                ...
                const className = classNames('cs-dropdown-label cs-inputtext', {
                    'cs-placeholder': label === null && props.placeholder,
                    'cs-dropdown-label-empty':
        ...
        const createClearIcon = () => {
            if (props.value != null && props.showClear && !props.disabled) {
                return (
                    <i
                        className="cs-dropdown-clear-icon csi csi-times"
                        onClick={clear}
                    ></i>
                );
            }
        ...
        const createDropdownIcon = () => {
            const iconClassName = classNames(
                'cs-dropdown-trigger-icon cs-clickable',
                props.dropdownIcon
            );
        ...
        const className = classNames(
            'cs-dropdown cs-component cs-inputwrapper',
            {
                'cs-disabled': props.disabled,
                'cs-focus': focusedState,
                'cs-dropdown-clearable': props.showClear && !props.disabled,
                'cs-inputwrapper-filled': ObjectUtils.isNotEmpty(props.value),
                'cs-inputwrapper-focus': focusedState || overlayVisibleState,
            },
            props.className
        );
...
const defaultProps = {
    ...
    dropdownIcon: 'csi csi-chevron-down',
    showOnFocus: false,
    onChange: null,
    onFocus: null,
    onBlur: null,
