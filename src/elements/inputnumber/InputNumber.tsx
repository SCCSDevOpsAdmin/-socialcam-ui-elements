...
export const InputNumber = React.memo(
    React.forwardRef((props: InputNumberProps, ref) => {
        ...
        const constructParser = () => {
            numberFormat.current = new Intl.NumberFormat(
                props.locale,
                getOptions()
            );
            const numerals = [
                    ...
                    if (selectionStart === selectionEnd) {
                        ...
                        if (isNumeralChar(deleteChar)) {
                            ...
                            if (_group.current.test(deleteChar)) {
                                ...
                                if (decimalLength) {
                                    //@ts-ignore
                                    $refs.input.$el.setSelectionRange(
                                        selectionStart + 1,
                                        selectionStart + 1
                                    );
                                } else {
        ...
        const validateValue = (value) => {
            ...
            if (props.max !== null && value > props.max && !props.blockMax) {
                return props.max;
            } else if (
                props.max !== null &&
                value > props.max &&
        ...
        const concatValues = (val1, val2) => {
            if (val1 && val2) {
                ...
                return decimalCharIndex !== -1
                    ? val1.split(_decimal.current)[0] +
                          val2.slice(decimalCharIndex)
                    : val1;
            }
        ...
        const createUpButton = () => {
            const className = classNames(
                'cs-inputnumber-button cs-inputnumber-button-up cs-button cs-button-icon-only cs-component',
                {
        ...
        const createDownButton = () => {
            const className = classNames(
                'cs-inputnumber-button cs-inputnumber-button-down cs-button cs-button-icon-only cs-component',
                {
                    'cs-disabled': props.disabled,
                },
        ...
        const createButtonGroup = () => {
            ...
            if (stacked) {
                return (
                    <span className="cs-inputnumber-button-group">
                        {upButton}
                        {downButton}
                    </span>
                );
        ...
        const className = classNames(
            'cs-inputnumber cs-component cs-inputwrapper',
            {
                'cs-inputwrapper-filled':
                    props.value != null && props.value.toString().length > 0,
                'cs-inputwrapper-focus': focusedState,
                'cs-inputnumber-buttons-stacked': stacked,
                'cs-inputnumber-buttons-horizontal': horizontal,
                'cs-inputnumber-buttons-vertical': vertical,
            },
            props.className
        );
...
const defaultProps = {
    ...
    incrementButtonIcon: 'csi csi-angle-up',
    decrementButtonIcon: 'csi csi-angle-down',
    locale: undefined,
