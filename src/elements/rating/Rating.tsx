...
export const Rating = React.memo(
    React.forwardRef((props: RatingProps, ref) => {
            ...
            return Array.from({ length: props.stars }, (_, i) => i + 1).map(
                (value) => {
                    const iconClassName = classNames('cs-rating-icon', {
                        'csi csi-star': !props.value || value > props.value,
                        'csi csi-star-fill': value <= props.value,
                    });
        ...
        const createCancelIcon = () => {
            if (props.cancel) {
                return (
                    <span
                        className="cs-rating-icon cs-rating-cancel csi csi-ban"
                        onClick={clear}
                        tabIndex={tabIndex}
