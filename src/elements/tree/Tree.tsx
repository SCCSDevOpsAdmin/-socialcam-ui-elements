...
export const Tree = React.memo(
    React.forwardRef((props: TreeProps, ref?) => {
        ...
        const validateDropPoint = (event) => {
            let _validateDrop = validateDrop(
        ...
        const createLoader = () => {
            if (props.loading) {
                const icon = classNames(
                    'cs-tree-loading-icon csi-spin',
                    props.loadingIcon
                );

                return (
                    <div className="cs-tree-loading-overlay cs-component-overlay">
                        <i className={icon} />
                    </div>
                );
            }
        ...
        const createFilter = () => {
            if (props.filter) {
                const value = ObjectUtils.isNotEmpty(filteredValue)
                    ...
                    : '';
                let content = (
                    <div className="cs-tree-filter-container">
                        <input
                            type="text"
                            value={value}
                            autoComplete="off"
                            className="cs-tree-filter cs-inputtext cs-component"
                            placeholder={props.filterPlaceholder}
                            onKeyDown={onFilterInputKeyDown}
                            onChange={onFilterInputChange}
                            disabled={props.disabled}
                        />
                        <span className="cs-tree-filter-icon csi csi-search"></span>
                    </div>
                );

                if (props.filterTemplate) {
                    const defaultContentOptions = {
                        className: 'cs-tree-filter-container',
                        element: content,
                        filterOptions: filterOptions,
                        filterInputKeyDown: onFilterInputKeyDown,
                        filterInputChange: onFilterInputChange,
                        filterIconClassName:
                            'cs-dropdown-filter-icon csi csi-search',
                        props,
                    };
        ...
        const createHeader = () => {
            if (props.showHeader) {
                ...
                if (props.header) {
                    const defaultContentOptions = {
                        filterContainerClassName: 'cs-tree-filter-container',
                        filterIconClasssName:
                            'cs-tree-filter-icon csi csi-search',
                        filterInput: {
                            className:
                                'cs-tree-filter cs-inputtext cs-component',
                            onKeyDown: onFilterInputKeyDown,
                            onChange: onFilterInputChange,
                        },
                        filterElement,
                ...
                return <div className="cs-tree-header">{content}</div>;
            }
        ...
        const createFooter = () => {
            ...
            return <div className="cs-tree-footer">{content}</div>;
        };

        const otherProps = ObjectUtils.findDiffKeys(props, defaultProps);
        const className = classNames('cs-tree cs-component', props.className, {
            'cs-tree-selectable': props.selectionMode,
            'cs-tree-loading': props.loading,
