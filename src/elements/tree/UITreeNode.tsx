...
export const UITreeNode = React.memo((props: any) => {
    ...
    const onNodeKeyDown = (event) => {
        ...
        if (!DomHandler.hasClass(nodeElement, 'cs-treenode')) {
            return;
        }
    ...
    const getParentNodeElement = (nodeElement) => {
        ...
        return DomHandler.hasClass(parentNodeElement, 'cs-treenode')
            ? parentNodeElement
            : null;
    };
    ...
    const onClick = (event) => {
        ...
        if (
            ...
            targetNode === 'BUTTON' ||
            targetNode === 'A' ||
            DomHandler.hasClass(event.target, 'cs-clickable')
        ) {
            return;
        }
    ...
    const propagateUp = (event) => {
        ...
        for (let child of props.node.children) {
            if (selectionKeys[child.key] && selectionKeys[child.key].checked)
                ...
                selectionKeys[child.key] &&
                selectionKeys[child.key].partialChecked
            )
        ...
        if (check && checkedChildCount === props.node.children.length) {
            ...
            if (
                ...
                (checkedChildCount > 0 &&
                    checkedChildCount !== props.node.children.length)
            )
                selectionKeys[props.node.key] = {
                    checked: false,
                    partialChecked: true,
    ...
    const onDropPointDragEnter = (event) => {
        if (
            ...
            DomHandler.addClass(event.target, 'cs-treenode-droppoint-active');
        }
    };
    ...
    const onDrop = (event) => {
        if (props.dragdropScope && props.node.droppable !== false) {
            DomHandler.removeClass(contentRef.current, 'cs-treenode-dragover');
            event.preventDefault();
            event.stopPropagation();
    ...
    const onDragEnter = (event) => {
        if (
            ...
            DomHandler.addClass(contentRef.current, 'cs-treenode-dragover');
        }
    };
    ...
    const createLabel = () => {
        let content = (
            <span className="cs-treenode-label">{props.node.label}</span>
        );

        if (props.nodeTemplate) {
            const defaultContentOptions = {
                onTogglerClick: onTogglerClick,
                className: 'cs-treenode-label',
                element: content,
                props,
                expanded,
            };
    ...
    const createCheckbox = () => {
        if (isCheckboxSelectionMode() && props.node.selectable !== false) {
            ...
            const className = classNames('cs-checkbox-box', {
                'cs-highlight': checked,
                'cs-indeterminate': partialChecked,
                'cs-disabled': props.disabled,
            });
            const icon = classNames('cs-checkbox-icon cs-c', {
                'csi csi-check': checked,
                'csi csi-minus': partialChecked,
            });
    ...
    const createIcon = () => {
        ...
        if (icon) {
            const className = classNames('cs-treenode-icon', icon);
    ...
    const createToggler = () => {
        const iconClassName = classNames('cs-tree-toggler-icon csi csi-fw', {
            'csi-chevron-right': !expanded,
            'csi-chevron-down': expanded,
        });
        let content = (
            <button
                type="button"
                className="cs-tree-toggler cs-link"
                tabIndex={-1}
        ...
        if (props.togglerTemplate) {
            const defaultContentOptions = {
                onClick: onTogglerClick,
                containerClassName: 'cs-tree-toggler cs-link',
                iconClassName: 'cs-tree-toggler-icon',
                element: content,
                props,
                expanded,
            };
    ...
    const createDropPoint = (position) => {
        if (props.dragdropScope) {
            return (
                <li
                    className="cs-treenode-droppoint"
    ...
    const createContent = () => {
        ...
        const className = classNames(
            ...
            {
                'cs-treenode-selectable':
                    props.selectionMode && props.node.selectable !== false,
                'cs-highlight': isCheckboxSelectionMode() ? checked : selected,
                'cs-highlight-contextmenu':
    ...
    const createChildren = () => {
        if (ObjectUtils.isNotEmpty(props.node.children) && expanded) {
            return (
                <ul className="cs-treenode-children" role="group">
